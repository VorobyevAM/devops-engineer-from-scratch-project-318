...
Git. Корневой `requirements.yml` — символическая ссылка на единственный файл
`ansible/requirements.yml`, а `playbook.yml` — небольшая совместимая точка
входа для автоматической проверки Hexlet. Рабочая конфигурация от этого не
дублируется. Конфигурации сервисов шаблонизированы, секреты в шаблонах
отсутствуют.
